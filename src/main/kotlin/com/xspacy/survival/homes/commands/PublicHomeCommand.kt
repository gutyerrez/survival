...
class PublicHomeCommand : CustomCommand("publica") {
    override fun onCommand(commandSender: CommandSender, args: Array<out String>) {
        ...
        if (home[HomesTable.public]) {
            return commandSender.spigot().sendMessage(ComponentBuilder("Esta home já é pública.").color(ChatColor.RED).build())
        }

        transaction {
            HomesTable.update({ HomesTable.id eq home[HomesTable.id]}) {
                it[HomesTable.public] = true
            }
        }

        return commandSender.spigot().sendMessage(ComponentBuilder("Pública \"${homeName}\" adicionada com sucesso.").color(ChatColor.GREEN).build())
    }
}